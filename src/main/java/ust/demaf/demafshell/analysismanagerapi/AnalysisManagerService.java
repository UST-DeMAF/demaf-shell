...
public class AnalysisManagerService {
  ...
  @Autowired private WebClient analysisManagerApiClient;
  ...
  public List<String> getRegisteredPlugins() {
    return analysisManagerApiClient
        .get()
        .uri("/plugins")
  ...
  /**
   * Starts the transformation process and then continously polls for the result. At success, the
   * result is presented in the terminal. If an error occures, an error message is printed to the
   * terminal.
   *
   * @param technology
   * @param location
   ...
   */
  public String runTransformationProcess(
      String technology, URL location, List<String> commands, List<String> options) {
    TechnologySpecificDeploymentModel tsdm =
        new TechnologySpecificDeploymentModel(technology, location, commands, options);
    try {
  ...
  /**
   * Start the transformation process by invoking the corresponding endpoint of the analysis manager
   * service.
   *
   * @param tsdm
   ...
   * It polls the endpoint in an interval given by delayInSeconds until the returned message
   ...
   */
  public String pollTransformationProcessStatusForResult(
      UUID transformationProcessId, long delayInSeconds) {
    try {
      ProcessStatusMessage message =
          analysisManagerApiClient
              .get()
  ...
  private void printTransformationResult(Result result) {
    ...
    terminal
        .writer()
        .println("Find the technology-agnostic deployment model under the following path:");
    ...
    terminal
        .writer()
        .println("Type Completeness Val1: " + result.getTypeCompletenessVal1() * 100 + "%");
    terminal
        .writer()
        .println("Type Completeness Val2: " + result.getTypeCompletenessVal2() * 100 + "%");
